...
public class Matrix{
	...
	static double determinant(int n, double[][] mdata){
		double det = 0.0;
		...
		else
		{
			for (int i = 0; i < n; ++i)
			{
				double[][] newSubMatrix = subMatrix(0, i, mdata, n);
				det += pow(-1.0, (double)i) * mdata[0][i] * determinant(n,newSubMatrix);
			}
		}
	...
	double[][] inverse(int n, double[][] mdata){
		...
		for (int i = 0; i < n; ++i)
		{
			for (int j = 0; j < n; ++j)
			{
				double[][] newSubMatrix = subMatrix(j, i, mdata, n);
				inv[i][j] = pow(-1.0, (double)i + j) * determinant(n, newSubMatrix) / det;
			}
		}
	...
	static double[][] subMatrix(int r, int c, double[][] mdata, int n)
	{
