...
{
    int32_t     I, J, K = 0;            //  Data indices
    int32_t     L = 0;                  //  Iterations
    int64_t     IRand, JRand, KRand;    //  Random indices
    int32_t     X = 0;
    ...
    //
    //  Measure time to generate random indices without data access
    //
    sleep( 1 );
    time( &StartTime );
    for( L = 0; L < NbrIterations; L++ ) {
    ...
    printf( ">>CacheBenchmark: Random indices, no data access; Start: %12ld; End: %12ld; Delta: %12ld\n",
           StartTime, EndTime, (EndTime - StartTime) );
    ...
    printf( ">>CacheBenchmark: Random indices, with data access; Start: %12ld; End: %12ld; Delta: %12ld\n",
           StartTime, EndTime, (EndTime - StartTime) );
